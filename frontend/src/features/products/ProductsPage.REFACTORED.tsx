...
export function ProductsPage() {
  ...
  // Fungsi Pembersihan Form & URL Preview Gambar untuk Mencegah Memory Leak
  ...
  const columns: Column<Product>[] = [
    {
      ...
      label: "Foto",
      render: (r) => (
        <div className="h-10 w-10 overflow-hidden rounded-md border border-border bg-surface-2">
          {r.image ? (
            <img
              src={r.image}
              alt={r.name}
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          <Button variant="primary" size="sm" onClick={() => openAdjust(r)}>
            Atur Stok
          </Button>
          <Button variant="secondary" size="sm" onClick={() => openMovements(r)}>
            Riwayat
          </Button>
          {isAdmin && (
            <button
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-text-secondary">
              Format yang didukung: JPG, PNG, WEBP. Otomatis diresize ke maks. 1024x1024px.
            </p>
          </div>
          {form.imagePreview && (
                    ...
                    render: (r) => (
                      <span
                        className={`tabular-nums ${
                          r.quantity_change > 0 ? "text-success" : "text-danger"
                        }`}
                      >
                        {r.quantity_change > 0 ? "+" : ""}
